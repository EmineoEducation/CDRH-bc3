...
//  DEPOT       : EmineoEducation/CDRH-bc3
//  TITRE       : CDRH     BLOC : bc3
//  CLE REDIS   : cdrh:bc3
...
function SlackApp({ openChannel }) {
  ...
  // `hydrated` interdit toute écriture avant d'avoir lu la session :
  ...
  useSlackEffect(() => {
    ...
    window.PAC_PERSIST.load().then(session => {
      ...
      if (sv && sv.history && Object.keys(sv.history).length) {
        ...
        // Rejoue le compteur : `livrableUnlocked` vit dans desktop.jsx
        // et n'est pas persisté de son côté.
  ...
  // F33 · n'amorce qu'après lecture de la session, et seulement si vide.
  useSlackEffect(() => {
    if (!hydrated) return;
  ...
  const sendMessage = async () => {
    ...
    setTimeout(async () => {
      const info = castOf(primaryName);
      try {
        // F36 · l'historique complet gonflait le prompt sans limite.
        const history = (chatHistory[primaryId] || []).filter(m => !m.typing).slice(-40).map(m =>
        ...
        const resp = await window.PAC_FETCH('/api/chat', {
          ...
          body: JSON.stringify({
            ...
            max_tokens: 500,
            system: buildSlackEvalPrompt(primaryName, primaryRole, cfg, D) + buildLivrableFactsBlock() + (window.PAC_CONV ? window.PAC_CONV() : '') + (window.__pacSessionBrief ? window.__pacSessionBrief() : ''),
