  ...
  const sendMessage = async () => {
    ...
    setTimeout(async () => {
      ...
      try {
        ...
        const history = (chatHistory[primaryId] || []).filter(m => !m.typing).slice(-16).map(m =>
        ...
        const resp = await window.PAC_FETCH('/api/chat', {
          ...
          body: JSON.stringify({
            ...
            max_tokens: 500,
            system: buildSlackEvalPrompt(primaryName, primaryRole, cfg, D) + buildLivrableFactsBlock() + (window.__pacSessionBrief ? window.__pacSessionBrief() : ''),
